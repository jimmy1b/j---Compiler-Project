...
public class Parser {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        	...
        	ArrayList<JFormalParameter> caught = new ArrayList<JFormalParameter>();
        	ArrayList<JStatement> catchBlocks = new ArrayList<JStatement>();
//            mustBe(CATCH);
            while(have(CATCH)) {
            	mustBe(LPAREN);
            	caught.add(formalParameter());
            	mustBe(RPAREN);
            	catchBlocks.add(statement());
            }
            JStatement finallyBlock = have(FINALLY) ? statement() : null;
            		
            return new JTryCatchStatement(line, tryBlock, catchBlocks, finallyBlock, caught);
        } else if (have(FOR)) {
        	mustBe(LPAREN);
...
//            mustBe(SEMI);
            ...
            mustBe(SEMI);
            JStatement inc = statementExpression();
            mustBe(RPAREN);
            JStatement body = statement();
            return new JForStatement(line, init, test, inc, body);
        } else if (have(RETURN)) {
            if (have(SEMI)) {
    ...
    private JExpression assignmentExpression() {
        ...
        if (have(ASSIGN)) {
            ...
            return new JBAndAssignOp(line, lhs, assignmentExpression());
        } else if (have(BOR_ASSIGN)) {
            return new JBOrAssignOp(line, lhs, assignmentExpression());
        } else if (have(BXOR_ASSIGN)) {
            return new JBXorAssignOp(line, lhs, assignmentExpression());
    ...
    private JExpression conditionalExpression() {
    	JExpression lhs = conditionalAndExpression(); //test
    	if (have(CONDITIONAL)) {
...
//            return new JIfStatement(line, test, consequent, alternate);
    ...
    private JExpression unaryExpression() {
        ...
        } else  if (have(COMPLIMENT)) {
            return new JComplimentOp(line, unaryExpression());
        } else{
